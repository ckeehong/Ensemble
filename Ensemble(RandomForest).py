# -*- coding: utf-8 -*-
"""
Created on Sat Jun  5 15:38:31 2021

@author: ckhon
"""
'''
https://pub.towardsai.net/fully-explained-ensemble-techniques-example-with-python-b83e50310841
'''
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                ...
                label = j)
plt.title('Random Forest Classifier (Training set) with Entropy')
plt.xlabel('Age')
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1],
                alpha=0.5, c = ListedColormap(('red', 'green'))(i),
                label = j)
plt.title('Random Forest Classifier (Test set) with Entropy')
plt.xlabel('Age')
plt.ylabel('Estimated Salary')
plt.legend()
plt.show()